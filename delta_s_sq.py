...
#
# To run, execute python delta_s_sq.py --wfn wfn.h5 --nv_in V --nc_in C
#
#
...
def case_one(occ_ref_alpha,occ_ref_beta,iv,ic,osqarray):
    ...
    # = - sum_{m in occ_alpha_ref} |\Delta_{m,ic}|^2
    #   + sum_{n in occ_beta_ref} |\Delta_{n,iv}|^2
    #   + |\Delta_{iv,ic}|^2
    ...
    for mm in occ_ref_alpha:
        print('osqarray')
        print(osqarray[mm-1,ic-1])
        temp = temp - osqarray[mm-1,ic-1]
            
    for nn in occ_ref_beta:
        print('osqarray')
        print(osqarray[nn-1,iv-1])
        temp = temp + osqarray[iv-1,nn-1]
        
    temp = temp + osqarray[iv-1,ic-1]

    return temp

def case_two_a(el,occ_ref_alpha,occ_ref_beta,iv,ic,ivp,icp,reoarray,imoarray):

    #  sum_{n in occ_beta_ref} <row iv | n > < n | col iv >
...
def case_two_b(el,occ_ref_alpha,occ_ref_beta,iv,ic,ivp,icp,reoarray,imoarray):
    ...
    #  - sum_{n in occ_ref_alpha} < n | col ic > < row ic | n > + < row iv=col iv | col ic > < row ic | col iv >
    ...
    temp = temp + reoarray[iv-1,icp-1]*reoarray[iv-1,ic-1] + imoarray[iv-1,icp-1]*imoarray[iv-1,ic-1]
    temp = temp - 1j*reoarray[iv-1,icp-1]*imoarray[iv-1,ic-1] + 1j*imoarray[iv-1,icp-1]*reoarray[iv-1,ic-1]
...
def case_three(el,occ_ref_alpha,occ_ref_beta,iv,ic,ivp,icp,reoarray,imoarray):
    ...
    temp = complex(reoarray[ivp-1,icp-1]*reoarray[iv-1,ic-1] + imoarray[ivp-1,icp-1]*imoarray[iv-1,ic-1])
    temp = complex(temp - 1j*reoarray[iv-1,ic-1]*imoarray[ivp-1,icp-1] + 1j*imoarray[iv-1,ic-1]*reoarray[ivp-1,icp-1])
...
def static_factor(occ_ref_alpha,occ_ref_beta):
    ...
    # New static factor (2022):
    # 1 - 2.0 * M_S(High_Spin Ref State)
